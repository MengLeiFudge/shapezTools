...
public class Shape {
    ...
    public Shape process(Operate operate, Shape... shape) {
     ...
     * @return 切割后的图形
     */
    private Shape cut(int... quadrants) {
     ...
     */
    private Shape cutBC(int filter) {
        int id = this.id;
        int ret = 0;
        while (id != 0) {
            // 取出指定层
     ...
     * @return 根据层筛选器切割后的最简图形的id
     */
    private Shape cutAX(int filter) {
        int ret = 0;
        // 空层数目
        int q = 0;
        for (int i = 0; i < 4; i++) {
     ...
     * @return 堆叠后的图形
     */
    private Shape stackOn(Shape bottomShape) {
        // 上层图形放到高16位
        int top = this.id << 16;
        ...
        return new Shape(bottom | (top & 0xFFFF));
    }
